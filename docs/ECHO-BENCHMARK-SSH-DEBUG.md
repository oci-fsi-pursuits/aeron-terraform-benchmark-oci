...
| Timeout; `echo-benchmark-debug` shows wrong/missing NIC | **`interface=`** does not match host (wrong prefix vs subnet mask, or wrong NIC name) | Default Terraform **`aeron_echo_udp_interface_prefix_length=auto`** tracks **`private_subnet_cidr`**. Override with an explicit prefix (e.g. `"16"`) **or** `aeron_echo_udp_named_interface` (e.g. from `ip -br a`). Template: `playbooks/roles/aeron-benchmark/templates/benchmark-config.env.j2`. |
