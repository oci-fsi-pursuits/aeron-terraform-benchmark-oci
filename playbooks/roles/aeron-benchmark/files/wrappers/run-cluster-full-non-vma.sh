...
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)-cluster-non-vma-full}"
...
FINAL_AGGREGATE="${FINAL_AGGREGATE:-${RUN_DIR}/cluster-full-non-vma-aggregate.csv}"
...
status "non-VMA full cluster benchmark started: ${RUN_DIR}"
...
if [[ "${NON_VMA_DISABLE_VMA_STATE:-1}" == "1" ]]; then
  status "disabling VMA benchmark state on nodes before non-VMA cluster run"
...
for rate_raw in "${rates[@]}"; do
  ...
  for mode_raw in "${modes[@]}"; do
    ...
    case "${mode}" in
      java|c) ;;
      *)
        status "skipping unsupported non-VMA cluster mode=${mode}"
        continue
        ;;
    esac
    ...
    status "running non-VMA cluster mode=${mode} rate=${rate}"
...
if [[ "${#archives[@]}" -gt 0 ]]; then
  status "aggregating ${#archives[@]} non-VMA cluster archive(s)"
...
if [[ "${run_failures}" -gt 0 ]]; then
  status "non-VMA full cluster benchmark completed with ${run_failures} failed run(s)"
  exit 1
fi

status "non-VMA full cluster benchmark completed successfully"
