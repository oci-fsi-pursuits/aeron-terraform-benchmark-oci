...
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)-echo-vma-full}"
...
FINAL_AGGREGATE="${FINAL_AGGREGATE:-${RUN_DIR}/echo-full-vma-aggregate.csv}"
...
status "VMA full echo benchmark started: ${RUN_DIR}"
...
for rate_raw in "${rates[@]}"; do
  ...
  for mode_raw in "${modes[@]}"; do
    ...
    case "${mode}" in
      ...
      *)
        status "skipping unsupported VMA mode=${mode}"
        continue
        ;;
    esac
    ...
    status "running VMA echo mode=${mode} rate=${rate}"
...
if [[ "${#archives[@]}" -gt 0 ]]; then
  status "aggregating ${#archives[@]} VMA archive(s)"
...
if [[ "${run_failures}" -gt 0 ]]; then
  status "VMA full echo benchmark completed with ${run_failures} failed run(s)"
  exit 1
fi

status "VMA full echo benchmark completed successfully"
